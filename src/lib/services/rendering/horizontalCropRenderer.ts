...
export const HORIZONTAL_OUTPUT: Dimensions = {
  width: 1920,
  height: 1080,
};

export interface HorizontalCropRenderOptions {
...
export interface HorizontalCropRenderResult {
  cropPixels: { x: number; y: number; width: number; height: number };
  qualityScale: number;
  qualityState: ReturnType<typeof qualityStateForScale>;
}

export const renderHorizontalCrop = ({
  canvas,
  source,
  sourceDimensions,
  controllerState,
  ...
  mirrored = false,
}: HorizontalCropRenderOptions): HorizontalCropRenderResult => {
  ...
  if (!context) {
    throw new Error('The horizontal preview could not create a 2D rendering context.');
  }
